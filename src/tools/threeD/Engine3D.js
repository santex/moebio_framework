...
Engine3D.prototype.line3DCoordinates = function(x0, y0, z0, x1, y1, z1) {
  ...
  if(prescale0 > 0 || prescale1 > 0) {
    if(prescale0 > 0 && prescale1 > 0) {
      polygon.push(new Point((this._basis[0].x * x0 + this._basis[1].x * y0 + this._basis[2].x * z0 + this._dx) * prescale0, (this._basis[0].y * x0 + this._basis[1].y * y0 + this._basis[2].y * z0) * prescale0));
      polygon.push(new Point((this._basis[0].x * x1 + this._basis[1].x * y1 + this._basis[2].x * z1 + this._dx) * prescale1, (this._basis[0].y * x1 + this._basis[1].y * y1 + this._basis[2].y * z1) * prescale1));
      return polygon;
    } else {
      ...
      if(prescale0 > 0) {
        polygon.push(new Point((p0B.x + this._dx) * prescale0, p0B.y * prescale0));
        polygon.push(new Point((pM.x + this._dx) * prescaleM, pM.y * prescaleM));
      } else {
        polygon.push(new Point((pM.x + this._dx) * prescaleM, pM.y * prescaleM));
        polygon.push(new Point((p1B.x +this._dx) * prescale1, p1B.y * prescale1));
      }
      return polygon;
    }
  }
